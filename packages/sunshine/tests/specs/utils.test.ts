...
import { timezonePattern, timezonePatternWithSeparator } from '../data/data';
...
describe('utils', () => {
  ...
  describe('isValidIsoDateWithTimezone', () => {
    ...
    it('should return false if the date is invalid', () => {
      expect(isValidIsoDateWithTimezone('2022-01-01X00:00:00-0600')).to.be
        .false;
    });
  });
  ...
  describe('extractOffsetFromISO', () => {
    it('should return the offset from the ISO string', () => {
      expect(extractOffsetFromISO('2022-01-01T00:00:00-06:00')).to.equal(
        '-06:00',
      );
    });
