import { DateTime, IANAZone, Settings, type Zone } from "luxon";
import type { Valid } from "luxon/src/_util.js";
import { DEFAULT_TIMEZONE } from "./data.js";
import type { ShunshineValidDateType, SunshineFalsyDateType } from "./types.js";
import { timestampIsMs, timestampIsSec } from "./utils.js";

Settings.throwOnInvalid = true;

export function setGlobalTimezone(
	primary: string,
...
export function sunshineDateTime(
	...
	if (date instanceof DateTime)
		return <DateTime<Valid>>date.setZone(Settings.defaultZone);
	...
	if (typeof date === "string") {
		return <DateTime<Valid>>(
			DateTime.fromISO(date, { zone: Settings.defaultZone })
		);
	}
	...
	if (typeof date === "number") {
		if (timestampIsMs(date))
			return <DateTime<Valid>>(
				DateTime.fromMillis(date, { zone: Settings.defaultZone })
			);
		if (timestampIsSec(date))
			return DateTime.fromSeconds(date, { zone: Settings.defaultZone });
	}
